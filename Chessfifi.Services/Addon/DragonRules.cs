using DomainPosition = Chessfifi.Domain.Position;
...
namespace Chessfifi.Services.Addon
{
    ...
    {
        public DragonRules(PieceTypesDto pieceTypes) : base()
        {
            FieldWidth = 10;
            FieldHeight = 8;
            ...
            foreach (var itm in pieceTypes.Value)
            {
                ...
                {
                    PawnTransforms.Add(itm.Key, (side) => { return new DomainPiece(side, itm.Value); });
                }
            }
        }
    }
